...
class StockExchangeState(StockExchangeStateInterface):
    ...
    def step(self, action):
        ...
        if action == Actions.Buy and not self.stock:
            cost = self.data["High"][self.offset] + self.data["High"][self.offset] * self.config.commission
        ...
        if action == Actions.Close and self.stock:
            ...
            self.account -= self.data["Low"][self.offset] * self.config.commission

        # Next step
        self.stock_history.append(self.stock * self.data["Low"][self.offset])
